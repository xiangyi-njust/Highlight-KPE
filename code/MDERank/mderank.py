...
MAX_LEN = 512
en_model = StanfordCoreNLP(r'../../../../Software/stanford-corenlp-full-2018-02-27', quiet=True)
...
def get_multi_field(data_path, labels_path, key='Highlight'):
    data = {}
    labels = {}
    ...
    if key == 'all':
        ...
        for i, context in enumerate(abs):
            contexts.append(context + ' ' + hts[i])
            # contexts.append(hts[i] + ' ' + context)
    else:
        contexts = df[key].tolist()
        contexts = [context.strip().replace('\n', '') for context in contexts]

    subset_links = list(link_to_keywords.keys())
    for i, context in enumerate(contexts):
        if links[i] not in subset_links:
            continue
        left = titles[i]
        data[left] = context
    ...
    for i, link in enumerate(links):
        ...
        left = titles[i]
        labels[left] = keywords

    return data, labels


def get_environment_process(data_path, labels_path, key='ab_content'):
    data = {}
    labels = {}
...
def get_elsevier_data(data_path, labels_path, key='Abstract'):
    # for data dict: the key is the filename, and the value is the context
    data = {}
    labels = {}

    # load data
    with open(labels_path, 'r') as f:
        link_to_items = json.load(f)
    ...
    contexts = []
    if key == 'all':
        # a_contexts = df['Abstract'].tolist()
        a_contexts = df['reserve_4'].tolist()
        h_contexts = df['Highlights'].tolist()
        for i, context in enumerate(a_contexts):
            # contexts.append(context)
            contexts.append(context + ' ' + h_contexts[i])
            # contexts.append(h_contexts[i] + ' ' + context)
    else:
        contexts = df[key].tolist()

    subset_links = list(link_to_items.keys())
    ...
    for i, link in enumerate(links):
        if links[i] not in subset_links:
            continue
        keywords = link_to_items[link]
        left = titles[i]
        labels[left] = keywords
    return data, labels


def get_elsevier_reshape_data(data_path, labels_path):
    # for data dict: the key is the filename, and the value is the context
    data = {}
    labels = {}
    ...
    key = 'reserve_1'
    a_contexts = df[key].tolist()
    h_contexts = df['New_Highlight'].tolist()
    for i, context in enumerate(a_contexts):
        contexts.append(context)
        # contexts.append(context + ' ' + h_contexts[i])
        # contexts.append(h_contexts[i] + ' ' + context)
...
def keyphrases_selection(doc_list, labels_stemed, labels, model, dataloader, log):
    ...
    for id, [ori_doc, masked_doc, doc_id] in enumerate(tqdm(dataloader, desc="Evaluating:")):
        ...
        with torch.no_grad():
            ...
            ori_outputs = model(input_ids=ori_input_ids, attention_mask=ori_attention_mask,
                                token_type_ids=ori_token_type_ids, output_hidden_states=True)
            masked_outputs = model(input_ids=masked_input_ids, attention_mask=masked_attention_mask,
    ...
    for i in range(len(doc_list)):
        ...
        generated_keyphrases[i] = Matched
    ...
    # save generated content for 


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_dir",
                        default='data/MultiField',
                        type=str,
                        # required=True,
    ...
    start = time.time()
    log = Logger(args.log_dir + args.dataset_name + '/a_h.log')
    ...
    elif args.dataset_name == "elsevier":
        # data, references = get_elsevier_data(args.dataset_dir + "/contexts.xlsx",
        #                                      args.dataset_dir + "/subset-keywords.json", key='New_Highlight')
        data, references = get_elsevier_reshape_data(args.dataset_dir + "/Lite-abstract.xlsx",
                                                     args.dataset_dir + "/subset-keywords.json")
    elif args.dataset_name == 'Elsevier-LIS':
        data, references = get_elsevier_data(args.dataset_dir + '/Texts-lite-abstract.xlsx',
                                             args.dataset_dir + '/Keywords.json', key='reserve_6')
    elif args.dataset_name == 'EP':
        data, references = get_environment_process(args.dataset_dir + '/Texts-lite-abstract.xlsx' ,
                                                   args.dataset_dir + '/EP_keywords.txt', key='all')
    elif args.dataset_name == 'MultiField':
        data, references = get_multi_field(args.dataset_dir + '/Texts-lite-abstract.xlsx',
                                           args.dataset_dir + '/keywords.json', key='all')
    else:
        data, references = get_inspec_data(args.dataset_dir)

    # data and references is the 
    docs_pairs = []
    doc_list = []
    labels = []
    labels_stemed = []
    ...
    # with open('../data/subset-candidate.json', 'r') as f:
    #     link_to_candidates = json.load(f)
    # cans = list(link_to_candidates.values())

    for idx, (key, doc) in enumerate(tqdm(data.items())):
